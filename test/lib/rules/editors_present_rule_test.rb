...
module Rules
  class EditorsPresentRuleTest < ActiveSupport::TestCase
    test "offenders returns empty string when no tournaments match criteria" do
      result = EditorsPresentRule.offenders
      assert_equal "", result
    end

    test "offenders includes tournament with no editors" do
    ...
    test "offenders excludes tournament with editors" do
      tournament = Tournament.create!(
        id: 2,
        title: "Tournament with editor",
    ...
    test "offenders excludes tournament with multiple editors" do
      tournament = Tournament.create!(
        id: 3,
        title: "Tournament with multiple editors",
        start_datetime: Time.zone.today + 1.day,
        maii_rating: true
      )
    ...
    test "message combines description and offenders" do
      ...
      expected_description = "Турниры, в которых не указаны редакторы"
      ...
      assert_equal expected_message, EditorsPresentRule.message
    end
